...
(** BIP173's 90-character cap. The BCH code only guarantees detection of up
    to 4 character errors within this length, so it is enforced in both
    directions rather than treated as advisory. *)
val max_length : int
...
(** [encode enc ~hrp ~data] with [data] as 5-bit groups.
    ...
    exceed {!max_length}. *)
val encode : encoding -> hrp:string -> data:int list -> string
...
val decode : string -> (encoding * string * int list, string) result
