...
exception
  Restart_full
      [@ocaml.doc
        " Exception indicating that fast deserialization did not succeed and \
...
type (_, _) sentinel_list =
  ...
  | NCons :
      (sentinel_field_spec list * 'a sentinel_getter) * ('b, 'c) sentinel_list
      -> ('a -> 'b, 'c) sentinel_list
...
let read_of_spec : type a b. (a, b) spec -> Field.field_type * (Reader.t -> a) =
  function
  ...
  | SInt32 ->
      (Varint, read_varint_unboxed ~f:(decode_zigzag_unboxed @@ Int32.of_int))
      ...
      ( Length_delimited,
        fun reader ->
          let Field.{ offset; length; data } =
            Reader.read_length_delimited reader
          in
...
let rec value : type a b. (a, b) compound -> a value = function
  | Basic_req ((index, _, _), spec) ->
      ...
      let getter = function
        | Some v -> v
        | None -> error_required_field_missing index spec
      in
  ...
  | Repeated ((index, _, _), spec, Not_packed) ->
      let read =
        read_field ~read:(read_of_spec spec) ~map:(fun vs v -> v :: vs)
      in
      Value ([ (index, read) ], [], List.rev)
  | Map ((index, _, _), (key_spec, value_compound)) ->
      let read_key =
        value (Basic ((1, "key", "key"), key_spec, default_of_spec key_spec))
      in
  ...
  | Oneof (oneofs, _index_f) ->
      let make_reader : a oneof -> a field_spec =
       fun (Oneof_elem ((index, _, _), spec, (constr, _destr))) ->
        let read =
          read_field ~read:(read_of_spec spec) ~map:(fun _ -> constr)
        in
        (index, read)
      in
...
let in_extension_ranges extension_ranges index =
  List.exists
    ~f:(fun (start, end') -> index >= start && index <= end')
    extension_ranges
...
let deserialize_full : type constr a.
 ...
 fun extension_ranges values constructor reader ->
  let rec make_sentinel_list : type a b.
      (a, b) value_list -> (a, b) sentinel_list = function
    | VNil -> NNil
    | VNil_ext -> NNil_ext
     ...
     | NCons ((fields, _), rest) ->
         let map =
           List.fold_left ~init:map
             ~f:(fun map (index, read) -> IntMap.add index read map)
             fields
         in
         create_map map rest
  in
  let rec apply : type constr a.
   ...
   fun extensions constr -> function
     | NNil -> constr
     | NNil_ext -> constr extensions
     | NCons ((_, get), rest) -> apply extensions (constr (get ())) rest
  in
  let rec read :
...
[@@ocaml.doc " Full (slow) deserialization. "]

let deserialize_fast : type constr a.
    extension_ranges -> (constr, a) value_list -> constr -> Reader.t -> a =
 fun extension_ranges values constr reader ->
  let rec read_fields : type a.
   ...
   fun extension_ranges tpe idx reader extensions v -> function
     | (index, read_f) :: _ as lst when idx = index -> (
         let v = read_f v reader tpe in
         match Reader.next_field_header reader with
         | Some (tpe, idx) ->
             read_fields extension_ranges tpe idx reader extensions v lst
         | None -> (None, extensions, v))
     | rest when in_extension_ranges extension_ranges idx -> (
         let extensions =
           (idx, Reader.read_field_content tpe reader) :: extensions
         in
         match Reader.next_field_header reader with
         | Some (tpe, idx) ->
             read_fields extension_ranges tpe idx reader extensions v rest
         | None -> (None, extensions, v))
     | _ :: rest ->
         read_fields extension_ranges tpe idx reader extensions v rest
  ...
  let rec apply : type constr a.
   ...
   fun constr extensions -> function
     | VNil -> constr
     | VNil_ext -> constr (List.rev extensions)
     | VCons (Value (_, default, get), vs) ->
         apply (constr (get default)) extensions vs
  in
  let rec read_values : type constr a.
      ...
      (constr, a) value_list ->
      a =
   fun extension_ranges next_field reader constr extensions vs ->
    ...
    | Some (tpe, idx) -> (
        ...
        | VCons (Value (fields, default, get), vs) ->
            let next_field, extensions, v =
              read_fields extension_ranges tpe idx reader extensions default
                fields
            in
            read_values extension_ranges next_field reader
              (constr (get v))
              extensions vs
        | VNil | VNil_ext -> raise Restart_full)
  in
