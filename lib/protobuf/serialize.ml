...
let field_type : type a b. (a, b) spec -> int = function
  | Int64 | UInt64 | SInt64 | Int32 | UInt32 | SInt32 | Int64_int | UInt64_int
  | Int32_int | UInt32_int | SInt64_int | SInt32_int | Bool | Enum _ ->
      0
...
[@@ocaml.doc
  " Optimized when the value is given in advance, and the continuation is \
...
let rec write : type a b. (a, b) compound -> Writer.t -> a -> unit = function
  ...
  | Basic_opt ((index, _, _), spec) -> (
      let write = write_field spec index in
      fun writer v -> match v with Some v -> write writer v | None -> ())
  | Oneof (oneofs, index_f) -> (
      let create_writer : type a. a oneof -> Writer.t -> a -> unit = function
        | Oneof_elem (field, spec, (_constr, destructor)) ->
            ...
            fun writer v -> write writer (destructor v)
      in
...
let rec serialize : type a. (a, unit) compound_list -> Writer.t -> a = function
  ...
  | Nil_ext extension_ranges ->
      fun writer extensions ->
        List.iter
          ~f:(function
            | index, field when in_extension_ranges extension_ranges index ->
