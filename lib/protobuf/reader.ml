...
type t = { mutable offset : int; end_offset : int; data : String.t }

let create ?(offset = 0) ?length data =
  let end_offset =
    match length with None -> String.length data | Some l -> offset + l
  in
...
(* REUNA PATCH -- the second hand edit in this vendored runtime, and a
   memory-safety fix.
   ...
   `count` comes from read_varint_unboxed, which is Int64.to_int of a varint an
   attacker chose. A large value makes `t.offset + count` overflow to a
   negative int, the comparison succeeds, and read_length_delimited then hands
   deserialize.ml an out-of-range length. That reaches String.unsafe_blit,
   which is exactly as unsafe as it says: the process segfaults reading far
   past the end of the buffer.

   85 bytes of random input reach it. `Result.catch` cannot help and neither
   can any caller-side guard, because a segfault is not an exception.

   Comparing against the remaining space instead cannot overflow: both offset
   and end_offset are in range for the buffer, so their difference is a
   non-negative int, and a negative count is rejected outright rather than
   treated as "plenty of room".

   Found by ocaml-tron's fuzz/fuzz_raw_data.ml. Report upstream and drop this
   once it lands. *)
let validate_capacity t count =
  match count >= 0 && count <= t.end_offset - t.offset with
  | true -> ()
...
let read_length_delimited t =
  ...
  let v =
    let open Field in
    { offset = t.offset; length; data = t.data }
  in
