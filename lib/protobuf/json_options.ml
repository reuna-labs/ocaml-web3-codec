...
[@@ocaml.doc
  ...
  \   If [enum_names] is true then enums are serialized as strings. If false \
   the integer value is used when serializing.\n\n\
  \   If [json_name] is true then serialization will use the json field names. \
   If false, the fields names will be used from the protofile as-is.\n\n\
  \   If [omit_default_values] is false then default scalar values will not be \
   emitted to the json. The default is to omit default values.\n"]
...
[@@ocaml.doc " Perfect hash function "]
