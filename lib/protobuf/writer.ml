...
[@@ocaml.doc
  ...
  \    This allows the field length to be statically allocated and written \
   later.\n\
...
let ensure_capacity ~size t =
  match t.data with
  | ({ offset; buffer } as elem) :: _ when Bytes.length buffer - offset >= size
    ->
      elem
  | tl ->
      let elem =
        { offset = 0; buffer = Bytes.create (max size t.block_size) }
      in
      t.data <- elem :: tl;
      elem
...
let write_const_value data t =
  ...
  Bytes.blit_string ~src:data ~src_pos:0 ~dst:elem.buffer ~dst_pos:elem.offset
    ~len;
  elem.offset <- elem.offset + len
[@@ocaml.doc " Direct functions "]
...
let write_field : t -> int -> Field.t -> unit =
 fun t index field ->
  let field_type, writer =
    ...
    | Fixed_32_bit v -> (5, write_fixed32_value v)
  in
...
let write_length_delimited_f ~write_f v t =
  ...
  let write_balanced v t =
    let sentinel =
      match t.data with
      | ({ offset; buffer } as sentinel) :: _
        when offset + length_delimited_size_field_length <= Bytes.length buffer
        ->
          sentinel
      | _ ->
          let sentinel =
  ...
  let write_speed v t =
    ...
    in
    let _ =
      write_delimited_field_length_fixed_size sentinel.buffer ~offset size
    in
    ()
  in
  let write_space v t =
    ...
    let () =
      ...
      | false ->
          ...
          sentinel.offset <-
            sentinel.offset
            - (offset + length_delimited_size_field_length - offset')
    in
    ()
  in
  match t.mode with
...
let dump t =
  ...
  List.init ~len:(String.length string_contents) ~f:(fun i ->
      Printf.sprintf "%02x" (Char.code string_contents.[i]))
  |> String.concat ~sep:"-"
