...
let get_key ~f ~default key = function
  | `Assoc l -> List.assoc_opt key l |> Option.map f |> Option.value ~default
...
let to_camel_case s =
  ...
  |> String.concat ~sep:"" |> String.uncapitalize_ascii

let duration_to_json json =
  let seconds =
    get_key "seconds" ~f:Deserialize_json.to_int64 ~default:0L json
  in
...
let timestamp_to_json json =
  ...
  |> List.rev |> List.tl |> List.rev |> String.concat ~sep:"-"
  |> fun s -> `String (s ^ "Z")
...
and write : type a b. Json_options.t -> (a, b) compound -> a -> field list =
 fun options -> function
  ...
  | Map (index, (key_spec, value_compound)) -> (
      let key = key ~json_names:options.json_names index in
      let json_of_key =
        json_of_spec { options with omit_default_values = false } key_spec
      in
      let json_of_value =
...
let serialize : type a.
 ...
 fun ~message_name spec ->
  let arr =
    Array.init (Json_options.max_int + 1) ~f:(fun i ->
        Lazy.from_fun (fun () ->
            serialize ~message_name (Json_options.of_int i) spec []))
  in
