...
module Make (T : T) = struct
  ...
  type (_, _) spec =
    ...
    | Message :
        (module Message with type t = 'a) T.message
        -> ('a, message) spec

  type _ oneof =
    | Oneof_elem :
        field * ('b, _) spec * (('b -> 'a) * ('a -> 'b)) T.oneof_elem
        -> 'a oneof
  ...
  type (_, _) compound =
    ...
    | Map :
        field * (('a, scalar) spec * ('b, 'c basic) compound) T.map
        -> (('a * 'b) list, _ any) compound
    | Oneof :
        ('a oneof list * ('a -> int)) T.oneof
        -> (([> `not_set ] as 'a), _ any) compound

  type (_, _) compound_list =
    | Nil : ('a, 'a) compound_list
    | Nil_ext : extension_ranges -> (extensions -> 'a, 'a) compound_list
    | Cons :
        ('a, _) compound * ('b, 'c) compound_list
        -> ('a -> 'b, 'c) compound_list
