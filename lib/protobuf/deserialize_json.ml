...
let read_map : type a b.
    read_key:(string -> a) -> read_value:(Json.t -> b) -> Json.t -> (a * b) list
    =
...
let duration_of_json json =
  ...
  with _ -> value_error "google.protobuf.duration" json
[@@ocaml.doc
...
let timestamp_of_json = function
  | `String timestamp ->
      ...
      let nanos =
        Ptime.frac_s t |> Ptime.Span.to_float_s |> Float.mul 1_000_000_000.0
        |> Int64.of_float
      in
      (seconds, nanos)
...
let from_camel_case s =
  ...
  let rec map = function
    ...
    | [] -> []
  in
  String.to_seq s |> List.of_seq |> map |> List.to_seq |> String.of_seq
  ...
  | Oneof (oneofs, _) ->
      let rec make_readers = function
        | Oneof_elem (index, spec, (constr, _)) :: rest ->
            ...
            let read_opt fields =
              match read fields with Some v -> Some (constr v) | None -> None
            in
      ...
      let rec find fields = function
        | [] -> `not_set
        | read_opt :: rest -> (
            match read_opt fields with Some v -> v | None -> find fields rest)
      in
      fun fields -> find fields readers

let rec deserialize : type constr a.
    (constr, a) compound_list -> constr -> fields -> a = function
  | Nil -> fun constr _json -> constr
  | Nil_ext _extension_ranges -> fun constr _json -> constr []
  | Cons (spec, rest) ->
 ...
 fun ~message_name spec constr ->
  ...
  fun json ->
    ...
    | `Assoc fields ->
        fields
        |> List.fold_left
             ~f:(fun map (key, value) -> FieldMap.add key value map)
             ~init:FieldMap.empty
        |> deserialize
